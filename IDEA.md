...
1. **TCC grant-holder** — a signed `.app` with the right `Info.plist` usage keys (`NSCalendarsUsageDescription`, etc.) can hold **Full Disk Access + Calendar**. Every agent + nano CLI it launches inherits those grants (this is exactly *why running the nano tools from kitty works* — kitty is a grant-holding app; a headless node daemon is not). Fixes Messages **and** Calendar — the latter isn't fixable by adding a binary to Full Disk Access.
...
## Open questions
- **Signing:** Developer ID + notarization (distributable to Johannes & co.) vs stable ad-hoc (Nathan-local only)? Ad-hoc is enough for a first cut; Developer ID for sharing.
